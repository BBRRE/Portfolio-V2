...
const Other = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsVisible(true);
    }, 100); // Delay to allow the initial render to happen
  ...
  return (
    <div className={`bg-[#F7F0D7] w-full min-h-screen p-8 transition-opacity duration-1000`}>
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
        <ImageWithDialog
          src={house}
          alt="House"
