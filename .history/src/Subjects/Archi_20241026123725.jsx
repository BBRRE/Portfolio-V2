...
const Arcchi = () => {
    ...
    const slides = [
      { src: archi, text: 'Description for Image 1' },
      { src: eeng, text: 'Description for Image 2' },
      { src: archi, text: 'Description for Image 3' },
    ];
  
    const [currentSlide, setCurrentSlide] = useState(0);
    const [fade, setFade] = useState(true); // Track fade-in/out
  
    // Handle next and previous slide navigation
    const nextSlide = () => {
      setFade(false); // Start fade-out
      setTimeout(() => {
        setCurrentSlide((currentSlide + 1) % slides.length); // Move to next slide// Start fade-in
      }, 500); // Set timeout to match fade-out duration
    };
  
    const prevSlide = () => {
      setFade(false); // Start fade-out
      setTimeout(() => {
        setCurrentSlide((currentSlide - 1 + slides.length) % slides.length); // Move to previous slide
