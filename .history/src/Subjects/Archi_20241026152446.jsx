...
const Arcchi = () => {
  ...
  const imageVariants = {
    initial: { x: 0 }, // Initial position next to text
    expanded: { x: 'calc(50% - 50%)', transition: { duration: 0.5 } } // Move to center
  };
      ...
      <div className='flex flex-col pt-[100px] gap-8 w-full'>
        <div className={`flex w-full mx-auto max-w-[1200px] h-[80vh]`} onClick={() => { setIsExpanded(!isExpanded); }}>
          ...
          <motion.img
            ...
            style={{ width: isExpanded ? '100%' : '66.67%', height: '100%' }} // Adjust width based on state
          />
